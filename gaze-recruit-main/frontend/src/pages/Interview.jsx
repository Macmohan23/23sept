...
const Interview = () => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        // Check authentication (local backend)
        ...
        if (!jobRole) {
          ...
          const profile = await apiGet('/api/profiles/me');
        ...
        // Load role-specific questions from backend
        const questionsData = await apiGet(`/api/questions?job_role=${encodeURIComponent(jobRole)}`);
  ...
  const completeInterview = async (finalAnswers) => {
    try {
      ...
      // If video recorded, upload to backend to get a path
      let videoPath = null;
      if (recordedBlob) {
        try {
          // useVideoRecording exposes recordedBlob; the upload is managed there. We'll reuse stopVideoRecording already called.
          // Here we construct a File and upload again if needed, but our hook provides uploadRecording.
        } catch {}
      }

      // Create interview record via backend
      const interview = await apiPost('/api/interviews', {
        score: totalScore,
        ...
        status: 'completed',
        job_role: questions[0]?.job_role || 'unknown',
        video_recording_url: videoPath
      });
      ...
      if (interview) {
        const payload = finalAnswers
          ...
          .filter(Boolean);
        if (payload.length) {
          await apiPost(`/api/interviews/${interview.id}/answers`, payload);
        }
      }
        ...
        <Card className="mb-6 p-6 bg-card/50 backdrop-blur">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              {speechSupported && !isAnswerRecording && (
