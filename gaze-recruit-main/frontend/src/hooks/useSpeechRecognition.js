...
export const useSpeechRecognition = () => {
  ...
  const shouldListenRef = useRef(false); // Source of truth for whether we intend to listen
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onerror = (event) => {
        ...
        // Auto-restart only if we still intend to listen
        if (shouldListenRef.current && (event.error === 'network' || event.error === 'audio-capture')) {
          setTimeout(() => {
      ...
      recognition.onend = () => {
        setIsListening(false);
        // If we still intend to listen, try to resume (handles spurious onend)
        if (shouldListenRef.current) {
          try {
            recognitionRef.current?.start();
          } catch (err) {
            // swallow; start may throw if already starting
          }
        }
      };
